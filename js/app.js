...
(function () {
        ...
        .map(
          (detail, index) => {
            ...
            <tr data-detail-id="${detail.id}" data-qte-retour="${getTotalReturnQuantity(detail)}" class="${rowClasses}"${enterAnimationStyle}>
              ...
              <td><button class="cell-input cell-input--compact-dynamic return-summary-button" data-col-key="qteRetour" type="button" data-return-open="${detail.id}" title="Voir / ajouter un retour">${escapeHtml(formatEditableQuantityValue(getTotalReturnQuantity(detail)))}</button></td>
  ...
  function formatHistoryActionWithSite(history) {
    const materialAction = formatMaterialHistoryAction(history);
    if (materialAction) {
